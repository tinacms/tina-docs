export default function getTableOfContents(markdown: any): { type: string; text: string }[] {
    ...
    const nodes = Array.isArray(markdown) ? markdown : Array.isArray(markdown.children) ? markdown.children : [];
  
    nodes.forEach(item => {
      if ((item.type === 'h2' || item.type === 'h3') && Array.isArray(item.children)) {
